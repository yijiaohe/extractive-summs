...
def GSresults(queries_file, output_file, check_directory):
    ...
    for i in range(len(tid_list)):
        ...
        sql = "SELECT resource_id from resources_annotations where topic_id = %d" % (tid_list[i])
        cursor.execute(sql)
        ...
        for j in range(len(final_list)):
            r = str(int(final_list[j][0]))
            file_path = os.path.join(check_directory, r + ".txt")
            print file_path
            if (os.path.isfile(file_path)):
                out_file.write(str(i+1).zfill(2)+"\t0"+'\t'+r+'\t1')

                if (j != len(final_list) - 1):                
                    out_file.write('\n')
            else:
                print "file not in directory: " + file_path
        print "topic " + str(i + 1) + "done"
        if (i != len(tid_list) - 1  and len(final_list) != 0):
            out_file.write('\n')
